...
public class StudentController {
    ...
    public ResponseEntity<?> saveStudent(@RequestBody StudentRequestDto studentRequestDto){
        try {
            ...
            return ResponseEntity.internalServerError().body("Some exception occured while save! ->"+e.getMessage());
        }
    }
    @GetMapping("/find/{id}")
    public ResponseEntity<?> findStudentById(@PathVariable int id){
        try {
            ...
            return ResponseEntity.internalServerError().body("Some exception occured while find by id : ->"+id);
        }
    }
    @GetMapping("/findAll")
    ...
    public ResponseEntity<?> deleteStudent(@PathVariable int id){
        try {
            ...
            return ResponseEntity.internalServerError().body("Some exception occured while delete with id! ->"+id);
        }
    }
